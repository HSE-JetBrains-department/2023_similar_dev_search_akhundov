...
class StargazersContext:
    ...
    def __init__(self, repo: str):
        ...
        :param repo: GitHub API repository notation (<user>/<repository_name> or <organization>/<repository_name>
        """
        self.repo = repo
...
class StarredReposContext:
    ...
    def __init__(self, user: str):
        ...
        """
        self.user = user
        # Set of GitHub repository notations (<user>/<repository_name>, <organization>/<repository_name>)
        # starred by the user
...
class PopularReposContext:
    ...
    def __init__(self,
        ...
        # Final dictionary of popular repositories (GitHub repository notation to count of common stargazers)
...
class GatherPopularReposStage(Stage[PopularReposContext]):
    ...
    def __init__(self,
        ...
        """
        self._context = PopularReposContext(source_repos, max_popular_repos_num, page_limit, api_tokens,
                                            starred_repos_save_directory, stargazers_save_directory)
    ...
    def _count_top_repos(self):
        """
        Firstly, we retrieve all the stargazers for the requested repositories, merging them to avoid duplicates.
        Next, we iterate through each stargazer and fetch their starred repositories, saving intermediate results
        whenever feasible.
        Then, we gather all the results and identify the top-n repositories that have been starred
        by the original stargazers
        :return: The list of top-n repositories, ranked by the number of stars received from the initial stargazers
        """
        ...
        return dict(Counter(repo_star_counter).most_common(self._context.max_popular_repos))
